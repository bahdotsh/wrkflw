...
lazy_static! {
    static ref MATRIX_PATTERN: Regex = Regex::new(r"\$\{\{\s*matrix\.([a-zA-Z0-9_]+)\s*\}\}").unwrap();
}
...
pub fn process_step_run(run: &str, matrix_combination: &Option<HashMap<String, Value>>) -> String {
    if let Some(matrix) = matrix_combination {
        ...
        MATRIX_PATTERN.replace_all(run, |caps: &regex::Captures| {
            let var_name = &caps[1];
            format!("\\${{{{ matrix.{} }}}}", var_name)
        }).to_string()
    }
...
mod tests {
    ...
    fn test_preprocess_simple_matrix_vars() {
        ...
        matrix.insert("node".to_string(), Value::Number(serde_yaml::Number::from(14)));
    ...
    fn test_preprocess_with_missing_vars() {
        ...
        assert_eq!(processed, "echo \"Running on ubuntu-latest with Node \\${{ matrix.node }}\"");
    }
    ...
    fn test_preprocess_preserves_other_text() {
        ...
        assert_eq!(processed, "echo \"Starting job\" && echo \"OS: ubuntu-latest\" && echo \"Done!\"");
    }
